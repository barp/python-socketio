...
class AsyncServer(base_server.BaseServer):
    ...
    async def _handle_disconnect(self, eio_sid, namespace, reason=None):
        ...
        await self._trigger_event(
            'disconnect', namespace, sid, disconnect_reason)
    ...
    async def _recovery_cleanup_task(self):
        """Background task to clean up expired recovery sessions."""
        while True:
            await self.sleep(60)  # Run every minute
